...
from dataset.data import read_image,ImageDataset,init_dataset
from dataset import get_trm,make_dataloader
...
def inference_val(args,cfg,origin_model,save_dir, k1=20, k2=6, p=0.3, use_rerank=False,use_flip=False,n_randperm=0,bn_keys=[]):
    ...
    for label_idx in range(len(unique_hist_labels)):    
        ...
        val_set = ImageDataset(hl_query_infos[label_idx] + hl_gallery_infos[label_idx], cfg, val_trm)
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img
def inference_samples(args,origin_model,  transform, batch_size, query_txt,query_dir,gallery_dir,save_dir,k1=20, k2=6, p=0.3, use_rerank=False,use_flip=False,max_rank=200,bn_keys=[]):
    seperate_update = True
    ...
    for label_idx in range(len(unique_hist_labels)):    
        print("==>predict for hist_label:",unique_hist_labels[label_idx])
        print("==>build dataloader..")
        image_set = TestDataset(hl_query_infos[label_idx] + hl_gallery_infos[label_idx],transform)
