...
from dataset.data import read_image,ImageDataset,SepNormImageDataset,init_dataset
from dataset import get_trm,get_trm_by_meanstd,make_dataloader,make_sepnorm_dataloader
from dataset.collate_batch import val_collate_fn
...
def inference_val(args,cfg,origin_model,save_dir, k1=20, k2=6, p=0.3, use_rerank=False,use_flip=False,n_randperm=0,bn_keys=[]):
    ...
    # seperate by hist_labels
    img_infos = dataset.query + dataset.gallery
    num_query = len(dataset.query)
    img_filenames = [info[0] for info in img_infos]
    
    print("predict hist_label")
    hist_labels = mmcv.track_parallel_progress(simple_hist_predictor_func, img_filenames, 6)
    ...
    for label_idx in range(len(unique_hist_labels)):    
        print("==> predict for hist_label:",unique_hist_labels[label_idx])
        print("==> build dataloader..")
        val_set = SepNormImageDataset(hl_query_infos[label_idx] + hl_gallery_infos[label_idx], cfg, val_trm0,val_trm1)
        print("==> using data trainsform..")
        print('valid transform0: \n{}'.format(val_set.transform0))
        print('valid transform1: \n{}'.format(val_set.transform1))
        ...
        bn_dataloader = DataLoader(
            val_set, batch_size=cfg.TEST.IMS_PER_BATCH , shuffle=True,
            # val_set, batch_size=64, shuffle=False,
            num_workers=4,
            collate_fn=val_collate_fn,
            drop_last = True
        )
        num_query = len(hl_query_infos[label_idx])
        print("==> model inference..")
        
        if seperate_update:
            print("==> copy model for every hist_label")
            model = copy.deepcopy(origin_model)
        model = model.to(device)
        # if unique_hist_labels[label_idx] >0:
...
class SepNormTestDataset(Dataset):
    """RoIs Person ReID Dataset"""

    def __init__(self, img_fnames,transform0=None, transform1=None):
    ...
    def __getitem__(self, index):
        ...
        if self.transform0 is not None:
            if histlabel:
                ...
                # self.l0_cnt += 1
            # if self.l0_cnt % 100 == 0:
                # print(self.l0_cnt,self.l1_cnt)
        return img
def inference_samples(args,origin_model, transform0,transform1, batch_size, query_txt,query_dir,gallery_dir,save_dir,k1=20, k2=6, p=0.3, use_rerank=False,use_flip=False,max_rank=200,bn_keys=[]):
    seperate_update = True
    ...
    if args.adabn_com and args.adabn:
        print("using common adabn")
        seperate_update = False

        image_set = SepNormTestDataset(query_list + gallery_list,transform0,transform1)
    ...
    for label_idx in range(len(unique_hist_labels)):    
        print("==> predict for hist_label:",unique_hist_labels[label_idx])
        print("==> build dataloader..")
        image_set = SepNormTestDataset(hl_query_infos[label_idx] + hl_gallery_infos[label_idx],transform0,transform1)
        dataloader = DataLoader(image_set,sampler = SequentialSampler(image_set),batch_size= batch_size,num_workers = 4)
        bn_dataloader = DataLoader(image_set,sampler = RandomSampler(image_set),batch_size= batch_size,num_workers = 4,drop_last = True)
        print("==> using data trainsform..")
        print('test transform0: \n{}'.format(image_set.transform0))
        print('test transform1: \n{}'.format(image_set.transform1))
        num_query = len(hl_query_infos[label_idx])
        ...
        for q_idx in range(num_q):
            filename = hl_query_infos[label_idx][q_idx].split('/')[-1]
            max_200_files = [hl_gallery_infos[label_idx][i].split('/')[-1] for i in max_200_indices[q_idx]]
            res_dict[filename] = max_200_files
    if use_rerank:
        save_fname = 'mgnsub_rerank.json'
    elif args.dba:
    ...
    if args.adabn:
        if args.adabn_com:
            if args.adabn_all:
                save_fname = 'comadabnall_'+save_fname
            else:
                save_fname = 'comadabn_'+save_fname
        else:
            if args.adabn_all:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--adabn_com",action='store_true',help='')
    ...
    assert cfg.INPUT.SEPNORM.USE == True
    ...
    if args.sub == False:
        ...
        inference_val(args,cfg,model,cfg.OUTPUT_DIR, args.k1,args.k2, args.lambda_value, \
                        use_rerank=args.rerank,use_flip=args.flip,n_randperm=cfg.TEST.RANDOMPERM,\
                        bn_keys = bn_keys)
    else:
        pass
        # print(transform)
        test_trm0 = get_trm_by_meanstd(cfg,mean=cfg.INPUT.SEPNORM.TEST_PIXEL_MEAN[0], std=cfg.INPUT.SEPNORM.TEST_PIXEL_STD[0], is_train=False)
        test_trm1 = get_trm_by_meanstd(cfg,mean=cfg.INPUT.SEPNORM.TEST_PIXEL_MEAN[1], std=cfg.INPUT.SEPNORM.TEST_PIXEL_STD[1], is_train=False)
 
        inference_samples(args,model, test_trm0,test_trm1 , 128,args.query_txt, query_dir,gallery_dir,cfg.OUTPUT_DIR, args.k1,args.k2, args.lambda_value,\
