...
class BaseTrainer(object):
    def __init__(self, cfg, model, train_dl, val_dl,
        ...
        if num_gpus > 1:
            ...
            self.scheduler = LRScheduler(base_lr=cfg.SOLVER.BASE_LR, step=cfg.SOLVER.STEPS,
                                         factor=cfg.SOLVER.WARMUP_FACTOR, warmup_epoch=cfg.SOLVER.WARMUP_EPOCH,
                                         warmup_begin_lr=cfg.SOLVER.WARMUP_BEGAIN_RL,
                                         warmup_mode=cfg.SOLVER.WARMUP_METHOD)
            lr = self.scheduler.update(0)
            self.optim = optim.SGD(self.model.parameters(), lr=lr, weight_decay=5e-4,
    ...
    def handle_new_epoch(self):
        ...
        self.logger.info('-' * 20)
        if self.train_epoch > 100 and self.train_epoch % self.checkpoint_period == 0:
            self.save()
        if (self.train_epoch > 100 and self.train_epoch % self.eval_period == 0) or self.train_epoch == 50 :
            # self.evaluate()
            pass
        self.train_epoch += 1
